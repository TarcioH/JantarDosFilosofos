...
public class Filosofo extends Thread {
    ...
    final int FAMINTO = 1;
    final int COMENDO = 2;
    private static long[] tempoEspera = new long[5];
    private static long[] tempoMaxEspera = new long[5];
    private static int[] vezesComeu = new int[5];

    public static void exibirRelatorio() {
        StringBuilder relatorio = new StringBuilder();
        for (int i = 0; i < 5; i++) {
            relatorio.append("Filósofo ").append(i).append(": ");
            relatorio.append("Vezes Comeu: ").append(vezesComeu[i]).append(", ");
            relatorio.append("Tempo Máximo de Espera: ").append(tempoMaxEspera[i]).append(" ns, ");
            relatorio.append("Média de Espera: ").append(vezesComeu[i] > 0 ? tempoEspera[i] / vezesComeu[i] : 0).append(" ns\n");
        }

        JOptionPane.showMessageDialog(null, relatorio.toString(), "Relatório do Jantar dos Filósofos", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    public void PegarGarfo() {
        ...
        TentarObterGarfos();
        long tempoEsperaAtual = System.nanoTime() - inicioEspera;
        tempoEspera[this.ID] += tempoEsperaAtual;
        if (tempoEsperaAtual > tempoMaxEspera[this.ID]) {
            tempoMaxEspera[this.ID] = tempoEsperaAtual;
        }
        vezesComeu[this.ID]++;
        // Após operar, volta o semáforo mutex para o estado normal
        // indicando que já realizou todos procedimentos na mesa
